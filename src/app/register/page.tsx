...
export default function RegisterPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (!r.ok) { setErr(data?.error ?? 'Falha no cadastro'); return; }
    // após cadastro, encaminha pro login
    router.push('/login');
  };
  ...
  return (
    <main className="min-h-screen grid place-items-center bg-gradient-to-br from-gray-50 via-white to-gray-100">
      <form onSubmit={onSubmit} className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        <h1 className="mb-4 text-2xl font-extrabold">Cadastro</h1>
        <div className="mb-3">
        ...
        <p className="mt-3 text-center text-sm">
          Já tem conta? <a href="/login" className="text-indigo-600 hover:underline">Entre</a>
        </p>
      </form>
    </main>
  );
